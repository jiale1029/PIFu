...
class TrainDataset(Dataset):
    ...
    def get_subjects(self):
        ...
        var_subjects = np.atleast_1d(
            np.loadtxt(os.path.join(self.root, "val.txt"), dtype=str)
        )
    ...
    def get_render(self, subject, num_views, yid=0, pid=0, random_sample=False):
        ...
        view_ids = [
            self.yaw_list[
                (yid + len(self.yaw_list) // num_views * offset) % len(self.yaw_list)
            ]
        ...
        for vid in view_ids:
            param_path = os.path.join(
                self.PARAM, subject, "%d_%d_%02d.npy" % (vid, pitch, 0)
            )
            render_path = os.path.join(
                self.RENDER, subject, "%d_%d_%02d.jpg" % (vid, pitch, 0)
            )
            mask_path = os.path.join(
                self.MASK, subject, "%d_%d_%02d.png" % (vid, pitch, 0)
            )
            ...
            if self.is_train:
                ...
                if self.opt.random_trans:
                    dx = random.randint(
                        -int(round((w - tw) / 10.0)), int(round((w - tw) / 10.0))
                    )
                    dy = random.randint(
                        -int(round((h - th) / 10.0)), int(round((h - th) / 10.0))
                    )
                else:
                    dx = 0
                    dy = 0
    ...
    def select_sampling_method(self, subject):
        ...
        random_points = (
            np.random.rand(self.num_sample_inout // 4, 3) * length + self.B_MIN
        )
        ...
        inside_points = (
            inside_points[: self.num_sample_inout // 2]
            if nin > self.num_sample_inout // 2
            else inside_points
        )
        outside_points = (
            outside_points[: self.num_sample_inout // 2]
            if nin > self.num_sample_inout // 2
    ...
    def get_color_sampling(self, subject, yid, pid=0):
        ...
        pitch = self.pitch_list[pid]
        uv_render_path = os.path.join(
            self.UV_RENDER, subject, "%d_%d_%02d.jpg" % (yaw, pitch, 0)
        )
        ...
        if self.num_sample_color:
            sample_list = random.sample(
                range(0, surface_points.shape[0] - 1), self.num_sample_color
            )
        ...
        samples = (
            torch.Tensor(surface_points).float()
            + torch.normal(
                mean=torch.zeros((1, normal.size(1))), std=self.opt.sigma
            ).expand_as(normal)
            * normal
        )
